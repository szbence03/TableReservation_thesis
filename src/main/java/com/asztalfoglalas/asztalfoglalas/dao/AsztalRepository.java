...
public interface AsztalRepository extends JpaRepository<Asztal, Integer> {
    ...
    @Query("SELECT a FROM Asztal a WHERE a.id NOT IN (SELECT f.asztal.id FROM Foglalas f WHERE mettol <= ?2 AND meddig >= ?1)")
    List<Asztal> getSzabadAsztalok(LocalDateTime mettol, LocalDateTime meddig);

    @Query("SELECT a FROM Asztal a WHERE a.id IN (SELECT f.asztal.id FROM Foglalas f WHERE mettol <= ?2 AND meddig >= ?1)")
